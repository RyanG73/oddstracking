...
try:
    event_df = pd.read_csv('CFP_events.csv')
...
try:
    history = pd.read_csv('CFB_history.csv')
except:
...
history.to_csv('CFB_history.csv',index=False)
event.to_csv('CFB_events.csv',index=False)
