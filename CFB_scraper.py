...
eventIDs = []
url = 'https://sportsbook.draftkings.com//sites/US-SB/api/v1/eventgroup/2/full?format=json'
...
try:
    event_df = pd.read_csv('output/CFB_events.csv')
...
try:
    history = pd.read_csv('output/CFB_history.csv')
except:
...
history.to_csv('output/CFB_history.csv',index=False)
event.to_csv('output/CFB_events.csv',index=False)
